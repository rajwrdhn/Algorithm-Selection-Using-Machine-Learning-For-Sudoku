...
import time
class Dpll():
    def __init__(self, df, size, name):
        self.size  = size
        self.board = df
        self.name = name
    
    def v(self, i,j,d, size):
        """
        i, j -> cell number
        d -> digit in the cell
    ...
    def clausessudoku(self, size):
        ...
        for i in range(1, size+1):
            for j in range(1, size+1):
                ...
                res.append([self.v(i, j, d, size) for d in range(1, size+1)])
                # does not denote two different digits at once 
                for d in range(1, size+1):
                    for dp in range(d + 1, size+1):
                        res.append([-self.v(i, j, d, size), -self.v(i, j, dp, size)]) 
        ...
        def valid(cells):
            ...
            for i, xi in enumerate(cells):
                for j, xj in enumerate(cells):
                    if i < j:
                        for d in range(1, size +1):
                            res.append([-self.v(xi[0], xi[1], d,size), -self.v(xj[0], xj[1], d, size)])
    ...
    def solve(self, grid, size):
        """
        solve a Sudoku grid inplace 
        """
        clauses = self.clausessudoku(size)
        #print(clauses)
        for i in range(1, size + 1):
            for j in range(1, size + 1):
                d = grid[i - 1][j - 1]
                ...
                if d:
                    clauses.append([self.v(i, j, d, size)])
        #print(clauses)
        #    solve the SAT problem
        #print("clauses",clauses)
    
        dpll = self.run_dpll(clauses)
        #print("dpll",dpll)
        countminus = 0
        countplus = 0 
        for i in dpll:
    ...
    def numberofclauses(self, size):
        clauses = self.clausessudoku(size)
        return len(clauses)
    
    def numbernegativeclauses(self):
        return 0 
    
    def numberpositiveclauses(self):
        return 0

    def numberofvars(self, size):
        return 0 
    def ratioclausestovars(self):
        return 0
    ...
    def run_dpll(self, clauses):
        d_level = 0 
        Grph = nx.DiGraph()
        [Grph,conflict] = self.BCP(clauses,Grph,d_level)
        ...
        while True: 
            d_level +=1
            [Grph, sat] = self.DECIDE(clauses, Grph, d_level)
            if (sat):
                node1 = self.find_node(Grph,'value',1)
                node0 = self.find_node(Grph,'value',0)
                ...
                while True:
                    ...
                    b_level = self.ANALYZE_CONFLICT(Grph)
                    if (b_level==0):
                        print('Unsatisfiable')
                        return [False]
                    else: 
                        d_level = b_level-1
                        Grph = self.BACK_TRACK(Grph, b_level, clauses)


    def find_node(self,G, attr, value):
        result = []
    ...
    def ANALYZE_CONFLICT(self, G):
        maxi = self.find_max(G,'d_level')
        return maxi

    def BACK_TRACK(self, G,b_level,cnf):
        dl_lst = self.find_node(G,'d_level',b_level)
    
        min_wl = self.find_max(G,'w_level')
        for i in dl_lst:
            tmp = G.nodes[i]['w_level']
            if(tmp<min_wl):
                min_wl = tmp
    
        wl_lst = self.find_node(G,'w_level',min_wl) 
